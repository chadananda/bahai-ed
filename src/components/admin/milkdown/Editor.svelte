<script>
 import { Editor, rootCtx, defaultValueCtx } from "@milkdown/core";
 import { menu, menuConfigCtx, defaultConfigItems, menuDefaultConfig } from '@milkdown-lab/plugin-menu'
 // import { block } from "@milkdown/plugin-block";
 import { commonmark } from '@milkdown/preset-commonmark';
 import { nord } from '@milkdown/theme-nord';
 import { history } from '@milkdown/plugin-history';
 import { clipboard } from '@milkdown/plugin-clipboard';
 import { upload } from '@milkdown/plugin-upload';
 import { trailing } from '@milkdown/plugin-trailing';
 import { gfm } from "@milkdown/preset-gfm";
//  import { asidePlugin } from './milkdown/asidePlugin.js'; // Import your custom node

//  import ./milkdown/asidePlugin;
 import { onMount } from 'svelte';
 import '@milkdown-lab/plugin-menu/style.css';

 export let post;
//  export let site;
//  export let translations;
//  export let authorList;
//  export let topicList;
//  export let categoryList;
//  export let languages;
//  export let client;

 function editor(node) {
  //  console.log('Setting up editor on node', node);

   let editorInstance;

   const startEditor = async () => {
     editorInstance = await Editor.make()
       .config(menuDefaultConfig)
       .config((ctx) => {
         ctx.set(rootCtx, node);
         ctx.set(defaultValueCtx, post?.body  || '');
        //  ctx.set(menuConfigCtx.key, {
        //   attributes: { class: 'milkdown-menu', 'data-menu': 'true' },
        //   items: defaultConfigItems,
        // })
       })
       .use(nord)
       .use(commonmark)
       .use(gfm)
       .use(history)
       .use(clipboard)
       .use(upload)
       .use(trailing)
       .use(menu)
      //  .use(asidePlugin)

       .create();
   };
   startEditor().catch(console.error);
   return {
     destroy() {
       editorInstance?.destroy();
     }
   };
 }

   // Simulated function to switch content based on language
   function loadContent(lang) {
     selectedLanguage = lang;
   }
   // Function to handle saving the post
   function savePost() {
    // console.log('Saving post...');
   }
   onMount(() => {
     //content = post?.body || ''; // Initialize with current post body or empty string
    //  console.log('hello world');

   });
  </script>



<div class="milkdownEditor no-prose !max-w-none" use:editor />

<svelte:head>
   <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
</svelte:head>

<style>
.milkdown-menu {
  --menu-bg: #2e3440;
  --menu-border: #434b5e;
  --menu-bg-hover: #81a1c11f;
  --menu-text-hover: #5e81ac;
  background-color: var(--menu-bg);
  overflow-x: scroll;
  border: 1px var(--menu-border) solid;
  border-radius: 0.8rem;
}

.milkdown-menu::-webkit-scrollbar {
  height: 2px;
  background-color: transparent;
}

.milkdown-menu::-webkit-scrollbar-thumb {
  border-radius: 999px;
  background-color: #81a1c161;
  border: 0px solid transparent;
  background-clip: content-box;
}

.milkdown-menu::-webkit-scrollbar-track {
  border-radius: 999px;
  background: transparent;
  border: 4px solid transparent;
}

.milkdown-menu ul {
  list-style: none;
}

.milkdown-menu button {
  border: none;
  display: inline-flex;
  align-items: center;
  padding: 0.5rem 0.5rem;
  margin: 8px 0;
  font-size: 1rem;
  background-color: transparent;
  color: white;
  cursor: pointer;
}

.milkdown-menu button:not(:only-child) {
  min-width: 160px;
  justify-content: space-between;
}

.milkdown-menu button:not(:only-child) > span {
  transition: transform 0.2s ease-in;
}

.milkdown-menu button:not(:only-child):where([aria-expanded='true']) > span {
  transform: rotate(-180deg);
}

.milkdown-menu button:hover,
.milkdown-menu button.active {
  background: var(--menu-bg-hover);
  color: var(--menu-text-hover);
}

.milkdown-menu div[role='separator'] {
  flex-shrink: 0;
  width: 1px;
  background-color: var(--menu-border);
  margin: 12px 16px;
  min-height: 24px;
}

.milkdown-menu ul[role='menubar'] {
  display: flex;
  align-items: center;
}

.milkdown-menu ul[role='menu'] {
  display: none;
  margin: 0;
  padding: 0;
  position: absolute;
  border: 1px solid var(--menu-border);
  background-color: var(--menu-bg);
  z-index: 100;
}

.milkdown-menu ul[role='menu'].show {
  display: block;
}

.milkdown-menu ul[role='menu'] li[role='menuitem'] {
  padding: 6px;
  color: white;
  cursor: pointer;
  padding: 0.75rem 1rem;
  font-size: 14px;
  min-width: 160px;
}

.milkdown-menu ul[role='menu'] li[role='menuitem']:is(:hover, :focus) {
  background-color: var(--menu-bg-hover);
  color: var(--menu-text-hover);
}
</style>
